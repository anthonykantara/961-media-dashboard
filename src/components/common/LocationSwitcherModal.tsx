...
export default function LocationSwitcherModal({ isOpen, onClose }: LocationSwitcherModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl bg-white rounded-2xl border border-gray-200 overflow-hidden z-10 flex flex-col max-h-[85vh] shadow-2xl">
        {/* Search Header */}
        <div className="p-3.5 sm:p-4 border-b border-gray-100 flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              autoFocus
              className="w-full pl-10 pr-9 py-2 bg-gray-50 border-0 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-[#FF0000] transition-all"
            />
            {searchQuery && (
              <button
                type="button"
          ...
          <button
            onClick={onClose}
            className="p-2 text-gray-400 hover:text-gray-700 rounded-xl hover:bg-gray-100 transition-colors cursor-pointer shrink-0"
            aria-label="Close"
          >
            <X className="w-4 h-4" />
          </button>
            ...
            <div>
              ...
              >
                ...
                >
                  ...
                  <div>
                    <span className={`text-sm font-medium ${activeLocationId === null ? 'text-[#FF0000] font-semibold' : 'text-gray-900'}`}>
                      Global
                    </span>
                    <span className="text-xs text-gray-400 ml-2 font-normal">All Destinations</span>
                  </div>
                </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {filteredGroups.map(({ country, cities }) => {
              ...
              return (
                <div 
                  key={country.id}
                  className="rounded-xl border border-gray-100 bg-gray-50/40 p-3.5 flex flex-col justify-between"
                >
                  <div>
                    {/* Country Header */}
                    <div className="flex items-center justify-between pb-2 mb-2 border-b border-gray-100">
                      <button
                        ...
                        className={`flex items-center gap-2 text-left cursor-pointer hover:text-[#FF0000] transition-colors group ${
                          isCountrySelected ? 'text-[#FF0000] font-semibold' : 'text-gray-900 font-medium'
                        }`}
                      >
                          ...
                          return (
                            <button
                              ...
                              className={`px-1.5 py-0.5 rounded text-[10px] font-medium transition-colors cursor-pointer ${
                                isLangActive
                                  ? 'bg-[#FF0000] text-white font-semibold'
                                  : 'bg-white/80 text-gray-600 hover:bg-gray-200 border border-gray-200/80'
                              }`}
                              title={langInfo.nativeName}
                            >
                              {langInfo.short}
                          ...
                          return (
                            <div
                              ...
                              className={`w-full flex items-center justify-between px-2 py-1.5 rounded-lg text-xs transition-colors ${
                                isCitySelected
                                  ? 'bg-red-50/80 text-[#FF0000] font-semibold'
                            ...
                            >
                              <button
                                ...
                                className="flex-1 text-left cursor-pointer hover:text-gray-900"
                              >
                                <span>{city.name}</span>
                              </button>
